...
Дата: 2026-09-09. Состояние: **предлагается; реализация не начата**.
...
| Диалоги | Список, даты, фильтры по виджету, поиск, сообщения и оценки; состав изменяющих операций ожидает ответа пользователя |
...
- Ответы оператора, массовое удаление истории, полное управление дополнительными
...
1. Ожидается ответ об операциях с чатами. Пока предлагаются просмотр/поиск/фильтры,
   удаление только после уточнения. Охват контента подтверждён: публичные страницы
   и записи; товары — отдельная интеграция.
2. В серверной задаче: откуда получать статус оплаты и какие операции сохранять
   при блокировке; это не выводится из проверенного `Project`.
3. До реализации: согласовать исходный профиль совместимости и необходимость
   multisite/отдельного домена админки, точную область прав и протокол подписи.
